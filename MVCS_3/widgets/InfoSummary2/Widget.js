...
define(['jimu/BaseWidget', 'jimu/LayerInfos/LayerInfos', 'jimu/utils', 'dojo/dom', 'dojo/dom-class', 'dojo/dom-construct', 'dojo/on', 'dojo/dom-style', 'dojo/_base/declare', 'dojo/_base/lang', 'dojo/_base/html', 'dojo/_base/Color', 'dojo/promise/all', 'dojo/_base/array', 'dojo/DeferredList', 'dojo/Deferred', 'dojo/query', "dojox/gfx/fx", 'dojox/gfx', 'dojo/_base/xhr', 'dijit/_WidgetsInTemplateMixin', 'dijit/_TemplatedMixin', 'jimu/dijit/Message', 'esri/graphic', 'esri/geometry/Point', 'esri/layers/FeatureLayer', /**/
'esri/symbols/SimpleMarkerSymbol', 'esri/symbols/PictureMarkerSymbol', 'esri/symbols/SimpleLineSymbol', 'esri/symbols/SimpleFillSymbol', 'esri/Color', 'esri/tasks/query', 'esri/tasks/QueryTask', "esri/tasks/FeatureSet", "esri/arcgis/utils", 'esri/symbols/jsonUtils', 'esri/request', "esri/renderers/SimpleRenderer", "esri/renderers/jsonUtils", './js/ClusterLayer', 'esri/layers/LayerDrawingOptions'], function (BaseWidget, LayerInfos, utils, dom, domClass, domConstruct, on, domStyle, declare, lang, html, dojoColor, all, array, DeferredList, Deferred, query, fx, gfx, xhr, _WidgetsInTemplateMixin, _TemplatedMixin, Message, Graphic, Point, FeatureLayer, /**/
SimpleMarkerSymbol, PictureMarkerSymbol, SimpleLineSymbol, SimpleFillSymbol, Color, Query, QueryTask, FeatureSet, arcgisUtils, jsonUtils, esriRequest, SimpleRenderer, jsonUtil, ClusterLayer, LayerDrawingOptions) {
  return declare([BaseWidget, _WidgetsInTemplateMixin], {
    ...
    _fillLineColor: function _fillLineColor(fill1, line1, colorLine, sizeLine, colorBackground) {
      return new SimpleRenderer(new SimpleFillSymbol(fill1, new SimpleLineSymbol(line1, this._lineColor(colorLine), sizeLine), this._lineColor(colorBackground)));
    },

    /* Configurar departamentos */
    _settingDepartamento: function _settingDepartamento(url, lineBorder, lineColor, lineSize, colorBackground) {
      ...
      this.featureLayerDepartamento.setRenderer(this._fillLineColor("solid", lineBorder, lineColor, lineSize, colorBackground));
    ...
    _settingProvincia: function _settingProvincia(url, lineBorder, lineColor, lineSize, colorBackground) {
      ...
      this.featureLayerProvincia.setRenderer(this._fillLineColor("solid", lineBorder, lineColor, lineSize, colorBackground));
    ...
    /* Configurar provincias */
    _settingDistrito: function _settingDistrito(url, lineBorder, lineColor, lineSize, colorBackground) {
      ...
      this.featureLayerDistrito.setRenderer(this._fillLineColor("solid", lineBorder, lineColor, lineSize, colorBackground));
    ...
    _reseatParam: function _reseatParam() {
      window.paramDepartamento = "";
      window.paramProvincia = "";
      window.paramDistrito = "";
    },

    _countProject: function _countProject() {
      var queryString = "";
      try {
        // QueryString
        ...
        /* Filtrado de capas */
        this.map.getLayer('VMVU_VISOR_DECRETO_DE_URGENCIA_9093').setDefinitionExpression('' + queryString);
        this.map.getLayer('VMCS_VISOR_DECRETO_DE_URGENCIA_5572').setDefinitionExpression('' + queryString);
      } catch (error) {
    ...
    _loadNoneGeneralLimite: function _loadNoneGeneralLimite() {
      try {
    ...
    _loadBlockGeneralLimite: function _loadBlockGeneralLimite() {
      try {
    ...
    _nameSelected: function _nameSelected(thisValue, name) {
      try {
        thisValue.parentNode.parentNode.childNodes[3].childNodes[3].innerText = name;
      } catch (error) {
        console.log("Error: _nameSelected " + error.name + " - " + error.message);
      }
    },

    _zoomDistrito: function _zoomDistrito(objectId) {
      try {
    ...
    _errorDistrito: function _errorDistrito(item) {
      try {
        var tagHtml = [];
        tagHtml.push('<li class="list-group-item"           data-objectId="" data-id="">          Error al cargar        </li>');
      ...
      try {
        ...
        if (item != null) {
          ...
          for (var i = 0; i < featuresLength; i++) {
            ...
            tagHtml.push('<li class="list-group-item"               data-objectId="' + featureAttributes[this.distrito.OBJECTID] + '"               data-distritoId="' + featureAttributes[this.distrito.IDDISTRITO] + '">              ' + featureAttributes[this.distrito.NAME_DISTRITO] + '            </li>');
          }
        } else {
          tagHtml.push('<li class="list-group-item">            <p class="font-italic">CARGANDO ....</p>          </li>');
        }
        this.featureLayerDistrito.clearSelection();
        this.CboDistrito.innerHTML = tagHtml.join("");
        // Cambia el nombre del limite seleccionado
    ...
    _zoomProvincia: function _zoomProvincia(objectId) {
      try {
    ...
    _errorProvincia: function _errorProvincia(item) {
      try {
        var tagHtml = [];
        tagHtml.push('<li class="list-group-item"             data-objectId="" data-id="">            Error al cargar\n          </li>');
      ...
      try {
        ...
        if (item != null) {
          ...
          for (var i = 0; i < featuresLength; i++) {
            ...
            tagHtml.push('<li class="list-group-item"                 data-objectId="' + this.featureAttributes[this.provincia.OBJECTID] + '"                 data-departamentoid="' + this.featureAttributes[this.provincia.IDDEPARTAMENTO] + '"                 data-provinciaId="' + this.featureAttributes[this.provincia.IDPROVINCIA] + '">                ' + this.featureAttributes[this.provincia.NAME_PROVINCIA] + '\n              </li>');
          }
    ...
    _zoomDepartamento: function _zoomDepartamento(objectId) {
    ...
    _errorDepartamento: function _errorDepartamento(item) {
      try {
        var tagHtml = [];
        tagHtml.push('<li class="list-group-item"           data-id=""> Error al cargar </li>');
      } catch (error) {
        console.log('Error: _errorDepartamento ' + error.name + ' - ' + error.message);
      }
    },

    _stringToHTML: function _stringToHTML() {
      ...
      try {
        ...
        if (item != null) {
          ...
          for (var i = 0; i < this.featuresLength; i++) {
            this.featureAttributes = item.features[i].attributes;
            tagHtml.push('<li class="list-group-item"                 data-objectId="' + this.featureAttributes[this.departamento.OBJECTID] + '"                 data-departamentoId="' + this.featureAttributes[this.departamento.IDDEPARTAMENTO] + '">                ' + this.featureAttributes[this.departamento.NAME_DEPARTAMENTO] + '\n              </li>');
          }
    ...
    _onClickLimpiarAmbito: function _onClickLimpiarAmbito() {
      try {
        /* Limpia DEPARTAMENTO */
    ...
    _onClickZoomGeneral: function _onClickZoomGeneral() {
      var nameLimite = "",
          dataLimite = "";
      try {
        thisValue = event.target;
    ...
    _onChangeLimitePolitico: function _onChangeLimitePolitico(e) {
      try {
    ...
    _callGeneralDistrito: function _callGeneralDistrito(inName, counter) {
      try {
        // DISTRITOS
        this.queryDistrito.where = this.distrito.NAME_DISTRITO + ' LIKE \'%' + inName + '%\'';
        this.queryTaskDistrito.execute(this.queryDistrito,
        ...
        function (item) {
          ...
          try {
            ...
            for (var r = 0; r < featuresLength; r++) {
              ...
              tagHtml.push('<li class="list-group-item"                   data-objectId="' + featureAttributes[this.distrito.OBJECTID] + '"                   data-id="' + featureAttributes[this.distrito.IDDISTRITO] + '"                   data-name="distrito" data-limite="' + featureAttributes[this.distrito.NAME_DISTRITO] + '">                   <span class="leyend-distrito" style="border: 2px solid ' + this.legendDistrito + ';"></span>&nbsp;                   ' + featureAttributes[this.distrito.NAME_DEPARTAMENTO] + ',                   Provincia ' + featureAttributes[this.distrito.NAME_PROVINCIA] + ',                   Distrito ' + featureAttributes[this.distrito.NAME_DISTRITO] + '\n                  </li>');
            }
    ...
    _callGeneralProvincia: function _callGeneralProvincia(inName, counter) {
      try {
        // PROVINCIAS
        this.queryProvincia.where = this.provincia.NAME_PROVINCIA + ' LIKE \'%' + inName + '%\'';
        // Ejecutar de PROVINCIAS
        this.queryTaskProvincia.execute(this.queryProvincia,
        ...
        function (item) {
          ...
          try {
            ...
            for (var i = 0; i < featuresLength; i++) {
              featureAttributes = item.features[i].attributes;
              tagHtml.push('<li class="list-group-item"                   data-objectId="' + featureAttributes[this.provincia.OBJECTID] + '"                   data-id="' + featureAttributes[this.provincia.IDPROVINCIA] + '"                   data-name="provincia" data-limite="' + featureAttributes[this.provincia.NAME_PROVINCIA] + '">\n                    <span class="leyend-provincia" style="border: 2px solid ' + this.legendProvincia + ';"></span>&nbsp;                    ' + featureAttributes[this.provincia.NAME_DEPARTAMENTO] + ',                     Provincia ' + featureAttributes[this.provincia.NAME_PROVINCIA] + '                  </li>');
            }
            this.ID_General.innerHTML += tagHtml.join("");
            this._tableResult(counter);
          } catch (error) {
    ...
    _callGeneralDepartamento: function _callGeneralDepartamento(inName, counter) {
      try {
        // DEPARTAMENTOS
        this.queryDepartamento.where = '        ' + this.departamento.NAME_DEPARTAMENTO + ' LIKE \'%' + inName + '%\'';
        // Ejecutar de DEPARTAMENTOS
        this.queryTaskDepartamento.execute(this.queryDepartamento,
        ...
        function (item) {
          ...
          try {
            ...
            for (var i = 0; i < featuresLength; i++) {
              featureAttributes = item.features[i].attributes;
              tagHtml.push('<li class="list-group-item"                   data-objectId="' + featureAttributes[this.departamento.OBJECTID] + '"                   data-id="' + featureAttributes[this.departamento.IDDEPARTAMENTO] + '"                   data-name="departamento" data-limite="' + featureAttributes[this.departamento.NAME_DEPARTAMENTO] + '">                  <span class="leyend-departamento" style="border: 2px solid ' + this.legendDepartamento + ';"></span>&nbsp;                  ' + featureAttributes[this.departamento.NAME_DEPARTAMENTO] + '                </li>');
            }
            this.ID_General.innerHTML += tagHtml.join("");
            this._tableResult(counter);
          } catch (error) {
    ...
    _onClickBuscarLugar: function _onClickBuscarLugar(param) {
      var inName = "",
          limitePolitico = "",
          limiteTodo = "";
      try {
        if (param != null) {
          ...
          if (inName) {
            ...
            limiteTodo = limitePolitico == 'departamento' ? this._callGeneralDepartamento(inName, 3) : limitePolitico == 'provincia' ? this._callGeneralProvincia(inName, 3) : limitePolitico == 'distrito' ? this._callGeneralDistrito(inName, 3) : 'todos';
            // Muestra TODOS
            if (limiteTodo == 'todos') {
            ...
            new Message({
              titleLabel: '<strong>                <i class="fa fa-exclamation-triangle" aria-hidden="true"></i>&nbsp;                COMPLETAR CAMPO</strong>',
              message: '<i class="fa fa-terminal" aria-hidden="true"></i>&nbsp;                Ingrese el <strong>L\xCDMITE POL\xCDTICO</strong> seg\xFAn la selecci\xF3n                 (<u>DEPARTAMENTOS</u>, <u>PROVINCIAS</u>, <u>DISTRITOS</u> o <u>TODOS</u>).'
            });
          }
        } else {
          this.ID_General.innerHTML = '<li class="list-group-item">Resultado de b\xFAsqueda</li>';
        }
      } catch (error) {
    ...
    // Limpiar resultado de lugar
    _onClickLimpiarLugar: function _onClickLimpiarLugar() {
      try {
    ...
    _activeTabSearch: function _activeTabSearch(positionTab) {
      try {
        var tabs4 = document.querySelectorAll('.jimu-widget-InfoSummary .tabs .tab');
        var tabsContent4 = document.querySelectorAll('.jimu-widget-InfoSummary .tab-content');
    ...
    postCreate: function postCreate() {
      ...
      this._settingDepartamento(this.urlDepartamento, this.legendDepartamentoBorder, this.legendDepartamento, parseFloat(this.legendDepartamentoLineSize), this.legendDepartamentoBackground);
      /* Configuracion provincia */
      this._settingProvincia(this.urlProvincia, this.legendProvinciaLineBorder, this.legendProvincia, parseFloat(this.legendProvinciaLineSize), this.legendProvinciaBackground);
      /* Configuracion distrito */
      this._settingDistrito(this.urlDistrito, this.legendDistritoLineBorder, this.legendDistrito, parseFloat(this.legendDistritoLineSize), this.legendDistritoBackground);
